...
const signUserUp = (req, res, handleErr) => {
  const username = req.body.username;
  const password = req.body.password;
  ...
  if (existingUser) {
    return handleErr({ status: 409, message: "Username is already taken" });
  }

  // Create a new user
  const newUser = {
    id: DB.users.length + 1, // This assumes that the id is always one more than the length of the users array
    username: username,
    password: password,
  };
